export const metrics = [
 ...
 { id: "transit", label: "Transit Access", short:"Transit access", note: "Scheduled Valley Metro service in and around the block group", direction:"Higher scores indicate more scheduled service relative to Maricopa County block groups." },
];
...
export const transitBands = [{min:80,label:"Very frequent",range:"80–100",color:"#155e75"},{min:65,label:"Frequent",range:"65–79",color:"#2387a3"},{min:50,label:"Typical",range:"50–64",color:"#62b8bd"},{min:35,label:"Limited",range:"35–49",color:"#b8d6c9"},{min:0,label:"Little scheduled service",range:"0–34",color:"#e1e4de"}];
...
export const metricBands=(key:string,measure='rate')=>key==='violent-crime'||key==='property-crime'?crimeBands(key,measure):key==='heat'?heatBands:key==='tree-canopy'?canopyBands:key==='transit'?transitBands:bands;
...
export const transitRating = (n:number|null)=>n==null?"Insufficient data":transitBands.find(b=>n>=b.min)?.label;
export const weights:Record<string,number>={income:25,poverty:25,vacancy:20,value:15,affordability:15};
export const fmt=(n:number|null,key:string)=>n==null?"Not available":key==='heat'?`${n.toFixed(1)}°F`:key==='transit'?n.toLocaleString():(key==='income'||key==='value')?new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}).format(n):`${n.toFixed(1)}%`;
...
export function missingReason(area:Record<string,any>,key:string):string {
 ...
 if(key==='transit')return 'Insufficient valid scheduled-service records for a transit access score.';
